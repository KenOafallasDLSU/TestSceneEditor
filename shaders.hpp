...
inline std::string loadShaderSource(std::string filepath)
{
...
inline GLShader loadShader(std::string shaderPath)
{
    static auto extToShaderType =
        std::unordered_map<std::string, std::pair<GLenum, std::string>>(
            { {".vert", {GL_VERTEX_SHADER, "vertex"}},
                {".frag", {GL_FRAGMENT_SHADER, "fragment"}},
                {".geom", {GL_GEOMETRY_SHADER, "geometry"}},
                {".comp", {GL_COMPUTE_SHADER, "compute"}} });

    const auto ext = shaderPath.substr(shaderPath.length() - 5);
    const auto it = extToShaderType.find(ext);
    if (it == end(extToShaderType)) {
        std::cerr << "Unrecognized shader extension " << ext << std::endl;
        throw std::runtime_error("Unrecognized shader extension " + ext);
    }
...
inline GLProgram compileProgram(std::vector<std::string> shaderPaths)
{
    GLProgram program;
